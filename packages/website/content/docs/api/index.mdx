...
<Tabs items={['ActionComponent.tsx', 'RouteComponent.tsx']}>
  <Tab value="ActionComponent.tsx" label="ActionComponent.tsx">
    ...
    export const ActionComponent = () => {
      const { execute, data, error } = useAction(greetingAction);
  ...
  <Tab value="RouteComponent.tsx" label="RouteComponent.tsx">
    ...
    export const RouteComponent = () => {
      const { execute, data, error } = useRoute('path/to/route');
...
`createRoute` is used to define server routes, which are traditional REST API endpoints (`GET`, `POST`, `PUT`, `DELETE`). These endpoints are useful for creating traditional REST API endpoints that can be consumed by other services (i.e. API clients, webhooks, etc).
